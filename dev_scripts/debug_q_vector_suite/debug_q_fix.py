...
def test_q_transformation(expt_file, refl_file):
    ...
    for i in range(n_test):
        ...
        try:
            ...
            print(f"  Crystal method: {q_crystal} (mag: {np.linalg.norm(q_crystal):.4f})")
        ...
        if 'xyzcal.mm' in reflections:
            try:
                ...
                print(f"  q_pixel:        {q_pixel} (mag: {np.linalg.norm(q_pixel):.4f})")
                print(f"  |lab - pixel|:   {np.linalg.norm(q_lab - q_pixel):.4f}")
                if q_crystal is not None:
                    print(f"  |crystal - pixel|: {np.linalg.norm(q_crystal - q_pixel):.4f}")
