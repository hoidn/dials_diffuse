...
def demo_still_processing_with_validation(
    ...
    try:
        ...
        print(f"     - Q-consistency: {extraction_config.q_consistency_tolerance_angstrom_inv} Å⁻¹")
        ...
        if not Path(image_path).exists():
            ...
            print("   📝 This demo will show you the interface without actual processing")
...
def demo_interface_without_dials(output_dir: str):
    ...
    from diffusepipe.crystallography.still_processing_and_validation import ValidationMetrics
...
def analyze_successful_outcome(outcome, output_path: Path):
    ...
    print(f"     • Q-consistency: {'✅ PASSED' if validation_metrics.get('q_consistency_passed') else '❌ FAILED'}")
    ...
    if mean_delta is not None or max_delta is not None:
        ...
        print(f"     • Reflections tested: {validation_metrics.get('num_reflections_tested', 'N/A')}")
    
    if validation_metrics.get('pdb_cell_passed') is not None:
        print(f"     • PDB cell check: {'✅ PASSED' if validation_metrics.get('pdb_cell_passed') else '❌ FAILED'}")
        print(f"     • PDB orientation: {'✅ PASSED' if validation_metrics.get('pdb_orientation_passed') else '❌ FAILED'}")
        if validation_metrics.get('misorientation_angle_vs_pdb'):
            print(f"     • Misorientation angle: {validation_metrics['misorientation_angle_vs_pdb']:.2f}°")
...
Examples:
  ...
  python demo_phase1_processing.py --image lys_nitr_10_6_0491_dials_processing/imported.cbf
    ...
    parser.add_argument(
        '--image',
        help='Path to CBF still image file'
    )
    parser.add_argument(
        '--pdb',
        help='Path to reference PDB file for validation'
    )
    parser.add_argument(
        '--output-dir',
