...
from diffusepipe.utils.cbf_utils import get_angle_increment_from_cbf, _parse_cbf_header_text
...
class TestGetAngleIncrementFromCBF:
    ...
    @patch('diffusepipe.utils.cbf_utils.dxtbx')
    def test_dxtbx_stills_data_success(self, mock_dxtbx):
        """Test successful dxtbx parsing for stills data (Angle_increment = 0.0)."""
        # Mock successful dxtbx loading for stills
        mock_image = MagicMock()
        mock_scan = MagicMock()
        mock_scan.get_oscillation.return_value = (0.0, 0.0)  # (start_angle, oscillation_width)
        mock_image.get_scan.return_value = mock_scan
        mock_dxtbx.load.return_value = mock_image
        
        result = get_angle_increment_from_cbf("test_stills.cbf")
        assert result == 0.0
        mock_dxtbx.load.assert_called_once_with("test_stills.cbf")

    @patch('diffusepipe.utils.cbf_utils.dxtbx')
    def test_dxtbx_sequence_data_success(self, mock_dxtbx):
        ...
        assert result == 0.1
        mock_dxtbx.load.assert_called_once_with("test_sequence.cbf")

    @patch('diffusepipe.utils.cbf_utils.dxtbx')
    def test_dxtbx_no_scan_object(self, mock_dxtbx):
    ...
    @patch('diffusepipe.utils.cbf_utils.dxtbx')
    def test_dxtbx_failure_fallback_to_text_parsing(self, mock_dxtbx):
        """Test fallback to text parsing when dxtbx fails."""
        # Mock dxtbx failure
        mock_dxtbx.load.side_effect = Exception("dxtbx failed")
...
"""
    ...
    @patch('diffusepipe.utils.cbf_utils.dxtbx')
    def test_dxtbx_scan_get_oscillation_fails(self, mock_dxtbx):
        """Test handling when scan.get_oscillation() raises an exception."""
        # Mock scan object that fails on get_oscillation
        mock_image = MagicMock()
        mock_scan = MagicMock()
        mock_scan.get_oscillation.side_effect = RuntimeError("Oscillation data unavailable")
        mock_image.get_scan.return_value = mock_scan
        mock_dxtbx.load.return_value = mock_image
...
"""
        ...
        try:
            ...
            result = _parse_cbf_header_text(temp_path)
            assert result is None  # Should fail with current implementation
        finally:
            os.unlink(temp_path)
...
class TestIntegrationCBFDetection:
    ...
    @patch('diffusepipe.utils.cbf_utils.dxtbx')
    def test_full_workflow_dxtbx_success(self, mock_dxtbx):
    ...
    @patch('diffusepipe.utils.cbf_utils.dxtbx')  
    def test_full_workflow_fallback_success(self, mock_dxtbx):
        """Test full workflow when dxtbx fails but text parsing succeeds."""
        mock_dxtbx.load.side_effect = Exception("Import error")
...
"""
    ...
    @patch('diffusepipe.utils.cbf_utils.dxtbx')
    def test_full_workflow_all_methods_fail(self, mock_dxtbx):
...
"""
        ...
        try:
            with pytest.raises(Exception):  # Should raise an exception when all methods fail
                get_angle_increment_from_cbf(temp_path)
        finally:
            os.unlink(temp_path)