...
from diffusepipe.orchestration.stills_pipeline_orchestrator import (
    StillsPipelineOrchestrator,
)
...
class TestStillsPipelineOrchestrator:
    ...
    def test_determine_processing_route_auto(
        self, mock_cbf_utils, orchestrator, basic_config
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            outcome = StillProcessingOutcome(
                ...
                extraction_outcome=OperationOutcome(
                    status="SUCCESS", output_artifacts={"diffuse_data_path": npz_path}
                ),
            )
    ...
    @patch(
        "diffusepipe.orchestration.stills_pipeline_orchestrator.DIALSStillsProcessAdapter"
    )
    @patch("diffusepipe.orchestration.stills_pipeline_orchestrator.DataExtractor")
    @patch.object(
        StillsPipelineOrchestrator,
        "_generate_bragg_mask",
        return_value="/fake/mask.pickle",
    )
    def test_process_single_still_success(
        self,
        mock_generate_mask,
        mock_extractor,
        mock_adapter,
    ...
    def test_process_stills_batch_no_phase3(
        self, mock_mkdir, mock_exists, orchestrator, basic_config
    ):
