...
class TestBraggMaskGenerator:
    ...
    def test_generate_bragg_mask_from_spots_success(self):
        ...
        with patch.object(self.generator.dials_adapter, 'generate_bragg_mask') as mock_generate:
    ...
    def test_generate_bragg_mask_from_shoeboxes_success(self, mock_maskcode, mock_flex):
        """Test successful Bragg mask generation using shoebox data."""
        # Arrange
        mock_detector = Mock()
        mock_panel = Mock()
        ...
        mock_reflections.__iter__ = Mock(return_value=iter([{"shoebox": Mock(), "panel": 0}]))
    ...
    @patch('diffusepipe.masking.bragg_mask_generator.flex')
    def test_generate_bragg_mask_from_shoeboxes_no_reflections(self, mock_flex):
        """Test shoebox-based generation with no reflections."""
        # Arrange
        mock_detector = Mock()
        mock_panel = Mock()
    ...
    def test_get_total_mask_for_still_exception_handling(self):
        ...
        mock_bragg_mask.__invert__ = Mock(side_effect=RuntimeError("Logical operation failed"))
    ...
    def test_process_reflection_shoebox_invalid_panel(self):
        ...
        with pytest.raises(Exception) as exc_info:
            self.generator._process_reflection_shoebox(
                reflection, 0, panel_masks, 0
            )
    ...
    def test_complete_workflow_option_a(self):
        ...
        with patch.object(self.generator.dials_adapter, 'generate_bragg_mask') as mock_generate:
            ...
            total_mask = self.generator.get_total_mask_for_still(
                ...
                global_pixel_mask=(mock_global_pixel_mask,)
            )
