...
class TestPixelMaskGenerator:
    ...
    def test_generate_static_mask_basic(self, mock_flex):
        ...
        assert len(result) == 1
        assert result[0] is mock_mask
        mock_flex.bool.assert_called()
    ...
    @patch('diffusepipe.masking.pixel_mask_generator.flex')
    def test_generate_dynamic_mask_no_images(self, mock_flex, mock_np):
        """Test dynamic mask generation with no representative images."""
        # Arrange
        mock_detector = Mock()
        mock_panel = Mock()
        mock_panel.get_image_size.return_value = (100, 50)
        ...
        mock_flex.bool.assert_called_with(mock_flex.grid.return_value, True)
    ...
    @patch('diffusepipe.masking.pixel_mask_generator.flex')
    def test_apply_beamstop_mask_circular(self, mock_flex):
        """Test application of circular beamstop mask."""
        # Arrange
        mock_panel = Mock()
        mock_panel.get_image_size.return_value = (6, 6)
        
        mock_initial_mask = Mock()
        mock_beamstop_mask = Mock()
        mock_inverted_mask = Mock()
        mock_result_mask = Mock()
        
        mock_flex.bool.return_value = mock_beamstop_mask
        mock_flex.grid.return_value = Mock()
        
        # Mock logical operations
        mock_beamstop_mask.__invert__ = Mock(return_value=mock_inverted_mask)
        mock_initial_mask.__and__ = Mock(return_value=mock_result_mask)
        
        beamstop = Circle(center_x=3, center_y=3, radius=1)
        
        # Act
        result = self.generator._apply_beamstop_mask(
            mock_panel, mock_initial_mask, beamstop
        )
        
        # Assert
        assert result is mock_result_mask
        mock_initial_mask.__and__.assert_called_once_with(mock_inverted_mask)
    
    @patch('diffusepipe.masking.pixel_mask_generator.flex')
    def test_apply_beamstop_mask_rectangular(self, mock_flex):
        """Test application of rectangular beamstop mask."""
        # Arrange
        mock_panel = Mock()
...
class TestMaskParameterClasses:
    ...
    def test_static_mask_params_creation(self):
        ...
        params = StaticMaskParams(
            beamstop=beamstop,
            untrusted_rects=rects,
            untrusted_panels=panels
        )
...
class TestUtilityFunctions:
    ...
    def test_create_rectangular_beamstop(self):
        ...
        beamstop = create_rectangular_beamstop(
            min_x=10, max_x=20, min_y=30, max_y=40
        )
