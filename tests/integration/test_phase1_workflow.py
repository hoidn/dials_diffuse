...
class TestPhase1Workflow:
    ...
    def test_complete_phase1_workflow_success(self):
        ...
        # Mock flex arrays for masks
        with patch('diffusepipe.masking.pixel_mask_generator.flex') as mock_pixel_flex:
            with patch('diffusepipe.masking.bragg_mask_generator.flex') as mock_bragg_flex:
                # Setup mock flex objects
                mock_pixel_mask = Mock()
                mock_pixel_mask.count = Mock(return_value=5000)  # Good pixels
                mock_pixel_flex.bool.return_value = mock_pixel_mask
                mock_pixel_flex.grid.return_value = Mock()
                
                mock_bragg_mask = Mock()
                mock_bragg_mask.count = Mock(return_value=500)  # Bragg pixels
                mock_bragg_flex.bool.return_value = mock_bragg_mask
                ...
                with patch.object(self.processor.adapter, 'process_still') as mock_process:
                    with patch.object(self.processor.validator, 'validate_geometry') as mock_validate:
                        ...
                        with patch.object(self.bragg_generator.dials_adapter, 'generate_bragg_mask') as mock_gen_mask:
                            mock_gen_mask.return_value = ((mock_bragg_mask,), True, "Mask generated")
                            ...
                            assert still_outcome.output_artifacts["validation_passed"] is True
                            
                            # Act - Step 2: Generate global pixel mask
                            global_pixel_mask = self.pixel_generator.generate_combined_pixel_mask(
                            ...
                            bragg_mask = self.bragg_generator.generate_bragg_mask_from_spots(
    ...
    def test_phase1_mask_compatibility_validation(self):
        ...
        with patch('diffusepipe.masking.bragg_mask_generator.flex') as mock_flex:
            ...
            # Act
            from diffusepipe.masking.bragg_mask_generator import validate_mask_compatibility
            is_compatible = validate_mask_compatibility(bragg_mask_tuple, pixel_mask_tuple)
    ...
    def test_phase1_component_integration_interfaces(self):
        ...
        assert hasattr(processor, 'validate_processing_outcome')
        assert hasattr(processor, 'adapter')
