...
class TestPhase2CoreFunctionality:
    ...
    def test_mask_total_2d_parameter_handling(self):
        ...
        result = extractor._validate_inputs(
            inputs, config, "/tmp/output.npz", mask_total_2d
        )
    ...
    def test_correction_factors_validation(self):
        ...
        invalid_cases = [
            (
                {"lp_mult": -0.5, "qe_mult": 0.8, "sa_mult": 1.0, "air_mult": 1.0},
                "not positive",
            ),
            (
                {"lp_mult": 15.0, "qe_mult": 0.8, "sa_mult": 1.0, "air_mult": 1.0},
                "unreasonable",
            ),
            (
                {"lp_mult": 1.0, "qe_mult": 5.0, "sa_mult": 1.0, "air_mult": 1.0},
                "unreasonable",
            ),
            (
                {"lp_mult": 1.0, "qe_mult": 0.8, "sa_mult": 1e7, "air_mult": 1.0},
                "unreasonable",
            ),
            (
                {"lp_mult": 1.0, "qe_mult": 0.8, "sa_mult": 1.0, "air_mult": 2.0},
                "unreasonable",
            ),
        ]
