    ...
    def generate_bragg_mask_from_shoeboxes(
        ...
        try:
            ...
            for panel in detector:
                ...
                height, width = panel_size[1], panel_size[0]  # (fast, slow) -> (slow, fast)
            ...
            if not reflections.has_key('shoebox'):
                raise BraggMaskError("Reflection table missing required 'shoebox' column")
            ...
            logger.info(f"Generated Bragg mask from shoeboxes, "
            ...
            if isinstance(e, BraggMaskError):
                ...
                raise BraggMaskError(f"Failed to generate Bragg mask from shoeboxes: {e}")
    
    def get_total_mask_for_still(
        self,
        bragg_mask: Tuple[object, ...],
        global_pixel_mask: Tuple[object, ...]
    ) -> Tuple[object, ...]:
        """
        ...
        for z in range(z1, z2):
            for y in range(y1, y2):
                for x in range(x1, x2):
                    ...
                    if (mask_code & MaskCode.Foreground or 
                        mask_code & MaskCode.Strong):
                        
                        # Project to 2D panel coordinates
                        # For stills, z should correspond to panel_id
                        if z == panel_id and 0 <= y < len(panel_masks[panel_id]) and 0 <= x < len(panel_masks[panel_id][0]):
...
def validate_mask_compatibility(
    ...
    try:
        ...
        if len(bragg_mask) != len(pixel_mask):
            logger.error(f"Panel count mismatch: {len(bragg_mask)} vs {len(pixel_mask)}")
            return False
        
        # Check dimensions for each panel
        for panel_idx, (bragg_panel, pixel_panel) in enumerate(zip(bragg_mask, pixel_mask)):
            if len(bragg_panel) != len(pixel_panel):
                logger.error(f"Panel {panel_idx} size mismatch: "
