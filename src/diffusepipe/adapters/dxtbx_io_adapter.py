...
class DXTBXIOAdapter:
    ...
    def load_experiment_list(self, expt_path: Union[str, Path]) -> object:
        ...
        try:
            ...
            # Import DXTBX components (delayed import)
            try:
                from dxtbx.model.experiment_list import ExperimentListFactory
            except ImportError as e:
                raise DIALSError(f"Failed to import DXTBX components: {e}")
            ...
            raise FileSystemError(f"Failed to load experiment list from {expt_path}: {e}") from e
    ...
    def load_reflection_table(self, refl_path: Union[str, Path]) -> object:
        ...
        try:
            ...
            raise FileSystemError(f"Failed to load reflection table from {refl_path}: {e}") from e
    
    def save_experiment_list(self, experiments: object, expt_path: Union[str, Path]) -> None:
        """
        ...
        try:
            ...
            logger.info(f"Successfully saved {len(experiments)} experiments to {expt_path}")
            
        except (FileSystemError, DIALSError):
            raise
        except Exception as e:
            raise FileSystemError(f"Failed to save experiment list to {expt_path}: {e}") from e
    
    def save_reflection_table(self, reflections: object, refl_path: Union[str, Path]) -> None:
        """
        ...
        try:
            ...
            logger.info(f"Successfully saved {len(reflections)} reflections to {refl_path}")
            
        except (FileSystemError, DIALSError):
            raise
        except Exception as e:
            raise FileSystemError(f"Failed to save reflection table to {refl_path}: {e}") from e