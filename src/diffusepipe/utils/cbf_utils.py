...
def get_angle_increment_from_cbf(image_path: str) -> Optional[float]:
    ...
    try:
        ...
        if scan is not None:
            try:
                ...
                if oscillation is None or len(oscillation) < 2:
                    logger.debug("Scan object returned None or incomplete oscillation data")
                    return 0.0
                angle_increment = oscillation[1]  # oscillation width per frame
                logger.info(f"Angle increment from dxtbx scan object: {angle_increment}°")
        ...
        logger.warning(f"dxtbx not available, falling back to text parsing for {image_path}")
        ...
        try:
            ...
            if fallback_result is not None:
                logger.info(f"Angle increment from header text parsing: {fallback_result}°")
                return fallback_result
            else:
                logger.warning(f"Could not determine Angle_increment for {image_path}. Defaulting to sequence processing. This may lead to incorrect results. Consider using 'force_processing_mode'.")
                return None
        except Exception as fallback_error:
            logger.error(f"All parsing methods failed for {image_path}: {fallback_error}")
            raise
...
def _parse_cbf_header_text(image_path: str) -> Optional[float]:
    ...
    angle_pattern = re.compile(
        r'^\s*#\s*Angle_increment\s+([+-]?\d*\.?\d+)\s*(?:deg\.?)?',
        re.IGNORECASE
    )
    ...
    try:
        with open(image_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            for line in content.split('\n'):
                ...
                if '_array_data.data' in line or 'Content-Type: application/octet-stream' in line:
                    break
                ...
                if match:
                    try:
                        ...
                        logger.debug(f"Found angle increment from header text: {angle_increment}°")
                        return angle_increment
                    except ValueError as e:
                        logger.warning(f"Could not convert angle increment '{match.group(1)}' to float: {e}")
                        continue
