...
# Maximum acceptable RMS misorientation between crystal models (degrees)
MAX_RMS_MISORIENTATION_DEG = 2.0
...
class GlobalVoxelGrid:
    ...
    def __init__(
        ...
        self.total_voxels = (
            (self.hkl_max[0] - self.hkl_min[0] + 1)
            * (self.hkl_max[1] - self.hkl_min[1] + 1)
            * (self.hkl_max[2] - self.hkl_min[2] + 1)
        )

        logger.info(
            f"Grid initialized: {self.total_voxels} voxels, "
    ...
    def _validate_config(self):
        ...
        if any(
            ndiv <= 0
            for ndiv in [self.config.ndiv_h, self.config.ndiv_k, self.config.ndiv_l]
        ):
    ...
    def _average_crystal_models(self):
        ...
        logger.info("Unit cell parameters before averaging:")
    ...
    def _check_orientation_spread(self, u_matrices):
        ...
        # Check if misorientation exceeds acceptable threshold
        if rms_misorientation > MAX_RMS_MISORIENTATION_DEG:
            raise ValueError(
                f"RMS misorientation {rms_misorientation:.2f}° exceeds threshold of "
                f"{MAX_RMS_MISORIENTATION_DEG}°. Input stills are not suitable for merging."
            )
        elif rms_misorientation > 1.0:  # Warning threshold
            logger.warning(
                f"RMS misorientation: {rms_misorientation:.2f}° - "
                "may cause some smearing in merged diffuse map"
            )
        else:
    ...
    def _determine_hkl_range(self):
        ...
        self.hkl_max = tuple(
            int(np.ceil(hkl_max_frac[i] * ndiv_list[i]) + buffer) for i in range(3)
        )
    ...
    def _calculate_averaging_diagnostics(self):
        ...
        self.diagnostics = {
            ...
            "total_diffuse_observations": sum(
                len(data.q_vectors) for data in self.diffuse_data
            ),
        }
